...
y_med_act = vertcat(ones(size(x_1,2),1),2*ones(size(x_2,2),1));

k = find(x_med==0);
miss = ones(size(x_med));
miss(k) = 0;

[A_med,A1_med,rmse1_med,rmse2_med] = matrix_completion(x_med,miss,'step1',1);
[y_labels_1_med,y_labels_2_med] = clustering(round(A_med),round(A1_med),2);
...
[A_small1,A1_small1,rmse1_small1,rmse2_small1] = matrix_completion(x_small1,miss,'step1',2);
...
[A_small2,A1_small2,rmse1_small2,rmse2_small2] = matrix_completion(x_small2,miss,'step1',2);
...
[A_large,A1_large,rmse1_large,rmse2_large] = matrix_completion(x_large2,miss,'step1',1);
...
y_act = vertcat(ones(size(x_1,2),1),2*ones(size(x_2,2),1),3 *ones(size(x_3,2),1),4 *ones(size(x_4,2),1));
% x = large_data_matrix;
k = find(x==0);
miss = ones(size(x));
miss(k) = 0;
